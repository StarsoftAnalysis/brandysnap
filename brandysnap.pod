...
Once you're happy than the script won't break anything, try it without the B<dry-run>.    
...
It creates a new snapshot every time.  What is more interesting is how many snapshots are KEPT.  
...
* Email notifications, e.g. if new snapshot can not be created e.g. disk full
...
Snapshots also get deleted as time passes.  If a day with four snapshots gets to be old enough to fall within a B<3w> spec, then the extra snapshots will be deleted.
...
Email messages can be sent if brandysnap fails.  This feature is available if the Perl
module Mail::SendEasy is installed.  The email implementation is very simple: it can do
SMTP authentication, but not SSL or any other form of security.  If security is important
for you, the best solution is for brandysnap to send its emails to a proper SMTP
server running on the same computer (i.e. 'localhost').
...
This options sets the verbosity of the printed output, on a scale from B<0> to B<3>.  Use higher values to see more about what brandysnap and rsync are doing.  B<verbose> can also be spelled B<verbosity> (default: B<1>)
...
Extra options to pass to rsync.  This is a convenience option to all you to add to or override
...
As of 28 November 2012 and version 0.2.9, the following issues and bugs are known.
