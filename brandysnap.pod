...
B<--restart> implies B<--snapshot> and B<--min-interval=0>.
...
Print a stack trace on error. (default: B<no>)
...
Option B<--remote-rsync-cmd> can only be specified once, and applies to all remote sources and directories.
