...
for rsync, not for any other command nor for the script itself.  (e.g. use B<--rsync-cmd = 'sudo rsync'> and B<--remote-rsync-cmd = 'sudo rsync'> to avoid the need for passwords [need more explanation here])
...
B<0-6d5,2-5w3,4m12> - keep up to 6 snapshots a day for five days, but consider days with no snapshots at all to be valid; them keep between 2 and 5 a week for three weeks, then keep snapshots for 12 months with 4 snapshots in each.
...
See the B<rsync-opts> option for ways to change this.
...
In B<dry-run> mode, brandysnap goes through the motions, but doesn't actually create or delete any snapshots.  The B<dry-run> option is also passed through to rsync. (default: B<nodry-run>)
...
Consider _all_ snapshots (oldest first) as expirable to make room when the destination is full. (default: B<noexpire-old>)
...
=item B<[no]stacktrace>

Print a stack trace on error. (default: B<nostacktrace>)
