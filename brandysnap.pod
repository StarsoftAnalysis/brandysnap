...
=item B<all-failed I<keep/rename/delete>>

What to do with the snapshot if none of the sources are copied successfully.
B<I<keep>> will mean that the incomplete snapshot will be considered complete, and is not recommended.
B<I<rename>> will cause the incomplete snapshot to be renamed with a 'B<-x>' suffix: this
...
=item B<some-failed I<keep/rename/delete>>

What to do with the snapshot if some of the sources are not copied successfully.  See B<--all-failed> for details. (default: B<rename>)
